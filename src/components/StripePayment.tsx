...
function PaymentForm({
  ...
  // Validate wallet shipping address against allowed countries + return shipping rates
      ...
      {!linkAuthenticated && (
        <>
          <ExpressCheckoutElement
            ...
            options={(() => {
              const builtRates = buildShippingRates(deliveryExpectations)
              // Shopify-style fallback: always provide at least one shipping rate so the
              // wallet (Google Pay / Apple Pay) opens its expanded sheet with the address
              // selector. Without rates, the wallet shows a compact sheet and silently
              // uses whatever address is on the Google/Apple account — the user can't
              // see or edit the shipping address. PDP (ProductExpressCheckout) does the
              // same fallback.
              const rates = (builtRates && builtRates.length > 0)
                ? builtRates
                : [{ id: 'standard', displayName: 'Envío estándar', amount: 0 }]
              // Ask the wallet to collect a shipping address ONLY when the on-page form
              // is empty/incomplete. If the form already has a complete address, run the
              // wallet in silent mode (it will use the form's address).
              // Defensive: AddressElement may report `complete=true` from hydrated defaults
              // even when the visible form is empty. Cross-check against the actual order
              // shipping address (line1) — if the order has no real address, force the
              // wallet to request one (expanded sheet).
              ...
              return {
                ...
                // Always restrict wallet's saved addresses to store's allowed countries,
                // even when we don't request shipping in the wallet (form-driven flow).
