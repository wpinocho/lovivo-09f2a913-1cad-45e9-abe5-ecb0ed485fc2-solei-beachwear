...
function PaymentRequestInner({
  ...
  // Missing-phone fallback (Shopify-style): Google Pay can return without phone
  // even when requestPayerPhone:true. Prompt the user before creating the order.
  const [phoneDialogOpen, setPhoneDialogOpen] = useState(false)
  const phoneResolverRef = useRef<{ resolve: (v: string) => void; reject: () => void } | null>(null)
  ...
  useEffect(() => {
    ...
    const handlePaymentMethod = async (ev: any) => {
      try {
        ...
        let payerPhone = ev.payerPhone as string | undefined
        const shipping = ev.shippingAddress || {}

        // Shopify-style fallback: if the wallet didn't return a usable phone
        // (Google Pay frequently doesn't even with requestPayerPhone:true),
        // pause and ask the user via dialog. If they cancel, abort the wallet.
        if (!isValidPhone(payerPhone)) {
          try {
            console.log('[PDP ExpressCheckout] phone missing — opening MissingPhoneDialog', { payerPhone })
            payerPhone = await requestMissingPhone()
          } catch {
            ev.complete('fail')
            toast({
