...
export const useCheckoutLogic = () => {
  ...
  useEffect(() => {
    ...
    // Validate country + state against shipping coverage v2 (ISO codes)
    const coverageV2Countries = shippingCoverageV2?.countries || [];
    console.log('🔍 Shipping coverage validation:', { shippingCoverageV2, coverageV2Countries, countryCode: address.countryCode, state: address.state });
    
    if (coverageV2Countries.length > 0) {
      const countryData = coverageV2Countries.find((c: any) => c.code === address.countryCode);
      
      if (!countryData) {
        setShippingError(`No realizamos envíos a ${address.country}. Por favor selecciona otra dirección.`);
        console.log('Shipping coverage blocked: country not found', { countryCode: address.countryCode, allowed: coverageV2Countries.map((c: any) => c.code) });
        return;
      }
      
      const allowedStates = countryData.states || [];
      if (allowedStates.length > 0 && !allowedStates.some((s: any) => s.name === address.state)) {
        setShippingError(`No realizamos envíos a ${address.state}, ${address.country}. Por favor selecciona otra dirección.`);
        console.log('Shipping coverage blocked: state not found', { state: address.state, allowed: allowedStates.map((s: any) => s.name) });
        return;
      }
    }
    
    // Valid address — clear any previous error
    setShippingError(null);
  ...
  }, [address.country, address.countryCode, address.state, address.postal_code, address.city, address.line1, address.line2, firstName, lastName, hasActiveCheckout, orderId, updateShippingAddress, shippingCoverageV2]);
  ...
  // Sending billing_address: null was causing 400 errors on orders that don't allow null fields.
  ...
  // Address fields are now validated by Stripe AddressElement (elements.submit() catches them)
